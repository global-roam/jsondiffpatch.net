...
namespace JsonDiffPatchDotNet.UnitTests
{
    ...
    public class PatchUnitTests
    {
        ...
        public void Patch_ObjectApplyEditText_Success()
        {
            ...
            Assert.AreEqual(right.Property("p").Value.ToString(), patched.Property("p").Value.ToString(), "String value");
        }

        [Test]
        public void Patch_ObjectApplyEditLongTextWithManyEdits_Success()
        {
            var jdp = new JsonDiffPatch(new Options()
            {
                 TextDiff= TextDiffMode.Simple,
                MinEfficientTextDiffLength = 10,
                DiffPatchMatchFactory = () => new DiffMatchPatch.diff_match_patch() { Patch_Margin = 1, Match_Distance = 1 }
            });
            var left = JObject.Parse(@"{ ""p"" : ""bla1h111111111111112312weldjidjoijfoiewjfoiefjefijfoejoijfiwoejfiewjfiwejfowjwifewjfejdewdwdewqwertyqwertifwiejifoiwfei"" }");
            var right = JObject.Parse(@"{ ""p"" : ""bla1h111111111111112312weldjidjxoijfoiewjfodgdgiefjefijfoejoijffiwoejfiewjfiwejfowjwifewsfejdewdwdewqwertyqwertifwiejifoiwfei"" }");
            var patch = jdp.Diff(left, right);
        ...
        [Test]
        public void Patch_ObjectApplyEditText_Success2()
        {
            var jdp = new JsonDiffPatch(new Options()
            {
                MinEfficientTextDiffLength = 10,
