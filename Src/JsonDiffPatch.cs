...
namespace JsonDiffPatchDotNet
{
    public class JsonDiffPatch
    {
        ...
        public JToken Diff(JToken left, JToken right)
        {
            ...
            {
                var dmp = _options.DiffPatchMatchFactory();
            ...
            if (patch.Type == JTokenType.Array)
            {
                ...
                else if (patchArray.Count == 3) // Delete, Move or TextDiff
                {
                    ...
                    else if (op == 2)
                    {
                        ...
                        string right;
                        if (left != null && left.Type == JTokenType.String)
                        {
                            var patchResult = dmp.patch_apply(patches, left.ToObject<string>());
                            var patchSuccesses = (bool[])patchResult[1];
                            if (!patchSuccesses.All(_ => _))
                            {
                                throw new Exception("Failed to apply diff_match_patch");
                            }
                            right = (string)patchResult[0];
                        }
                        else
                        {
                            //var allDiffs = patches.SelectMany(p => p.diffs).ToList();
                            //right = dmp.diff_text2(allDiffs);
                            if (patches.Count != 1)
                                throw new InvalidDataException("Invalid textline");
                            right = dmp.diff_text2(patches[0].diffs);
                        }
        ...
        public JToken Unpatch(JToken right, JToken patch)
        {
            ...
            if (patch.Type == JTokenType.Array)
            {
                ...
                else if (patchArray.Count == 3) // Delete, Move or TextDiff
                {
                    ...
                    {
                        //not sure that this can be reliably done.
                        //throw new InvalidOperationException("Cannot Unpatch when patch contains diff_patch_match patches.");
                        var dmp = _options.DiffPatchMatchFactory();
                        List<Patch> patches = dmp.patch_fromText(patchArray[0].ToObject<string>());

                        //var allDiffs = patches.SelectMany(p => p.diffs).ToList();
                        //string left = dmp.diff_text1(allDiffs);
